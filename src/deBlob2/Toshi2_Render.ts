...
import { TRB_TCMD_MeshInfo, TRB_TMeshResource } from "./Toshi2";
...
class TRB_MeshInfo_Data {
    ...
    constructor (device: GfxDevice, cache: GfxRenderCache, coalescedBufferss: GfxCoalescedBuffersCombo[], public meshInfo: TRB_TCMD_MeshInfo) {
...
export class TRB_MeshResource_Data {
    ...
    constructor (device: GfxDevice, cache: GfxRenderCache, public meshResource: TRB_TMeshResource) {
        if (!this.meshResource.mesh_info)
            return;

        const loadedVertexDatas: LoadedVertexData[] = [];

        function collectLodInfo(): void {
            if (!meshResource.mesh_info)
                return;

            for (let i = 0; i < meshResource.mesh_info.length; i++)
            for (let j = 0; j < meshResource.mesh_info[i].mesh_data.length; j++)
                loadedVertexDatas.push(meshResource.mesh_info[i].mesh_data[j].loadedVertexData);
        }
        ...
        for (let i = 0; i < this.meshResource.mesh_info.length; ++i) {
            const mesh_info = this.meshResource.mesh_info[i];
            this.mesh_infos.push(new TRB_MeshInfo_Data(device, cache, coalescedBuffers, mesh_info));
        }
    }
...
class TRB_MeshInfo_Instance {
    ...
    public draw(device: GfxDevice, renderInstManager: GfxRenderInstManager, viewerInput: ViewerRenderInput, mesh_resource: TRB_MeshResource_Instance, position: vec3 | null = null): void {
        ...
        if (!skeleton) {
            if (this.data.meshInfo.attributes_valid) {
                console.warn("Valid mesh without skeleton data.");
            }
            return;
        }
        ...
        for (let i = 0; i < this.data.meshInfo.mesh_data.length; ++i) {
            ...
            const display_list = this.data.meshInfo.display_lists[i];
            const joint_weights = this.data.meshInfo.joint_weights;
            const matrix_indices = display_list.matrix_indices!;
            const matrix_index_count = display_list.index_count;

            if (joint_weights.length <= 0) {
                if (matrix_index_count <= 0) {
                    const dst = packetParams.u_PosMtx[0];
                    if (position) {
                        mat4.translate(dst, dst, position);
                    }
                    mat4.mul(dst, joint_matrices[0], /*/mat4.create()); //*/skeleton.joints[0].unknown2);
                    if (position) {
                        mat4.translate(dst, dst, position);
                    }
                    mat4.mul(dst, viewerInput.camera.viewMatrix, dst);
                } else {
                    for (let j = 0; j < matrix_index_count; ++j) {
...
export class TRB_MeshResource_Instance {
    ...
    public draw(device: GfxDevice, renderInstManager: GfxRenderInstManager, viewerInput: ViewerRenderInput): void {
        if (!this.visible)
            ...
            const sphere = this.data.meshResource.bounding_sphere;
            const position = /*sphere ? vec3.fromValues(sphere[0], sphere[1], sphere[2]) :*/ null;
    
            for (let i = 0; i < this.data.mesh_infos.length; ++i) {
            this.mesh_infos[i].draw(device, renderInstManager, viewerInput, this, position);
        }
    }
}
