...
import { GfxDevice, GfxTexture, GfxSampler, GfxCullMode, GfxFrontFaceMode } from "../gfx/platform/GfxPlatform";
...
import { HSD_TExp, HSD_TExpList, HSD_TExpTev, HSD_TExpColorIn, HSD_TExpColorOp, HSD_TExpAlphaIn, HSD_TExpAlphaOp, HSD_TExpOrder, HSD_TEInput, HSD_TExpCnst, HSD_TExpCnstVal, HSD_TEXP_TEX, HSD_TEXP_RAS, HSD_TExpCnstTObj, HSD_TExpGetType, HSD_TExpType, HSD_TExpCompile } from "../SuperSmashBrosMelee/SYSDOLPHIN_TExp";
import { colorNewCopy, White, Color, colorCopy, OpaqueBlack, TransparentBlack, colorFromHSL, Magenta } from "../Color";
import { TRB_LodInfo, TRB_MeshInfo, TRB_TMeshResource } from "./Toshi";
...
function randomColor(): Color {
    let color: Color = colorNewCopy(White);
    const hue = Math.random();
    colorFromHSL(color, hue, 0.7, 0.6);

    return color;
}

function randomGrayColor(): Color {
    let color: Color = colorNewCopy(White);
    const lightness = Math.random() + 0.125;
    colorFromHSL(color, 0.0, 0.0, lightness);

    return color;
}

function defaultMObj() : HSD_MObj {
    const renderMode: HSD_RenderModeFlags = HSD_RenderModeFlags.DIFFUSE_MODE_MAT0;

    const tobj: HSD_TObj[] = [];

    // HSD_Material
    const ambient = colorNewCopy(TransparentBlack);
    const diffuse = randomColor();
    const specular = colorNewCopy(TransparentBlack);
    let alpha: number = 1.0;
    let shininess: number = 0.0;

    // HSD_PEDesc
    let peFlags: HSD_PEFlags = HSD_PEFlags.ENABLE_COMPARE | HSD_PEFlags.ENABLE_ZUPDATE;
...
class TRB_MeshInfo_Data {
    public shapeHelpers: GXShapeHelperGfx[] = [];
    public mobj: HSD_MObj_Data | null = null;

    constructor (device: GfxDevice, cache: GfxRenderCache, coalescedBufferss: GfxCoalescedBuffersCombo[], public meshInfo: TRB_MeshInfo) {
        this.mobj = new HSD_MObj_Data(device, cache, defaultMObj());

        for (let i = 0; i < this.meshInfo.mesh_data.length; ++i) {
            const mesh_data = this.meshInfo.mesh_data[i];
            const coalescedBuffers = coalescedBufferss.shift()!;
            this.shapeHelpers.push(new GXShapeHelperGfx(device, cache, coalescedBuffers.vertexBuffers, coalescedBuffers.indexBuffer, mesh_data.loadedVertexLayout, mesh_data.loadedVertexData));
        }
    }
...
class TRB_LodInfo_Data {
    public mesh_infos: TRB_MeshInfo_Data[] = [];

    constructor (device: GfxDevice, cache: GfxRenderCache, coalescedBufferss: GfxCoalescedBuffersCombo[], public lodInfo: TRB_LodInfo) {
        for (let i = 0; i < this.lodInfo.mesh_info.length; ++i) {
            const mesh_info = this.lodInfo.mesh_info[i];
            this.mesh_infos.push(new TRB_MeshInfo_Data(device, cache, coalescedBufferss, mesh_info));
        }
    }

    public destroy(device: GfxDevice): void {
        for (let i = 0; i < this.mesh_infos.length; i++)
            this.mesh_infos[i].destroy(device);
    }
}

export class TRB_MeshResource_Data {
    public lod_infos: TRB_LodInfo_Data[] = [];
    public coalescedBuffers: GfxBufferCoalescerCombo;

    constructor (device: GfxDevice, cache: GfxRenderCache, public meshResource: TRB_TMeshResource) {
        if (!this.meshResource.lod_info)
            return;

        const loadedVertexDatas: LoadedVertexData[] = [];

        function collectLodInfo(): void {
            if (!meshResource.lod_info)
                return;

            for (let i = 0; i < meshResource.lod_info.length; i++)
            for (let j = 0; j < meshResource.lod_info[i].mesh_info.length; j++)
            for (let k = 0; k < meshResource.lod_info[i].mesh_info[j].mesh_data.length; k++)
                loadedVertexDatas.push(meshResource.lod_info[i].mesh_info[j].mesh_data[k].loadedVertexData);
        }
        ...
        for (let i = 0; i < this.meshResource.lod_info.length; ++i) {
            const lod_info = this.meshResource.lod_info[i];
            this.lod_infos.push(new TRB_LodInfo_Data(device, cache, coalescedBuffers, lod_info));
        }
    }

    public destroy(device: GfxDevice): void {
        this.coalescedBuffers.destroy(device);
        for (let i = 0; i < this.lod_infos.length; i++)
            this.lod_infos[i].destroy(device);
    }
}
...
class HSD_MObj_Instance {
    ...
    private setupChannelMode(mb: GXMaterialBuilder): void {
        ...
        if (!!(rendermode & HSD_RenderModeFlags.DIFFUSE)) {
            ...
            mb.setChanCtrl(GX.ColorChannelID.COLOR0, true, GX.ColorSrc.REG, matSrc, 0x01, GX.DiffuseFunction.CLAMP, GX.AttenuationFunction.SPOT);
...
class TRB_MeshInfo_Instance {
    private mobj: HSD_MObj_Instance | null = null;

    public visible: boolean = true;

    constructor (public data: TRB_MeshInfo_Data) {
        // TODO: Handle material animations
        if (this.data.mobj !== null)
            this.mobj = new HSD_MObj_Instance(this.data.mobj, new HSD__TexImageDataCache());
    }
    ...
    public draw(device: GfxDevice, renderInstManager: GfxRenderInstManager, viewerInput: ViewerRenderInput, mesh_resource: TRB_MeshResource_Instance, position: vec3 | null = null): void {
        ...
        const skeleton = mesh_resource.data.meshResource.skeleton;
        if (!skeleton)
            return;
        ...
        materialParams.u_Lights[0].Color = colorNewCopy(Magenta);
        materialParams.u_Lights[0].DistAtten = vec3.fromValues(1.0, 1.0, 1.0);
        materialParams.u_Lights[0].Direction = vec3.fromValues(0.0, 1.0, 1.0);

        const template = renderInstManager.pushTemplateRenderInst();
        this.mobj.setOnRenderInst(device, renderInstManager.gfxRenderCache, template);

        const joint_matrices = mesh_resource.joint_matrices;

        packetParams.clear();

        for (let i = 0; i < this.data.meshInfo.mesh_data.length; ++i) {
            const mesh_data = this.data.meshInfo.mesh_data[i];
            const shapeHelper = this.data.shapeHelpers[i];

            // Calculate the draw matrices.

            const joint_weights = this.data.meshInfo.joint_weights;
            const matrix_indices = this.data.meshInfo.display_lists[i].matrix_indices!;

            if (joint_weights.length <= 0) {
                for (let j = 0; j < 1; ++j) {
                    const weight_index = matrix_indices[j];
                    if (weight_index < 0 || weight_index >= 255)
                        continue;

                    const dst = packetParams.u_PosMtx[j];
                    mat4.mul(dst, joint_matrices[weight_index], /*/mat4.create()); //*/skeleton.joints[weight_index].unknown2);
                    if (position) {
                        mat4.translate(dst, dst, position);
                    }
                    mat4.mul(dst, viewerInput.camera.viewMatrix, dst);
                }
            }
            else {
                for (let j = 0; j < matrix_indices.length; ++j) {
                    const weight_index = matrix_indices[j];
                    if (weight_index < 0 || weight_index >= 255)
                        continue;

                    const weight_info = joint_weights[weight_index];

                    const dst = packetParams.u_PosMtx[j];
                    dst.fill(0);

                    const weight_count = weight_info.joint_indices[0];
                    for (let k = 0; k < weight_count; ++k) {
                        const index = weight_info.joint_indices[k + 1];
                        const weight = weight_info.joint_weights[k];

                        mat4.mul(scratchMatrixEnv, joint_matrices[index], skeleton.joints[index].unknown2);
                        mat4.multiplyScalarAndAdd(dst, dst, scratchMatrixEnv, weight);
                    }
            ...
            megaStateFlags.frontFace = GfxFrontFaceMode.CCW;
...
class TRB_LodInfo_Instance {
    private mesh_infos: TRB_MeshInfo_Instance[] = [];

    public visible: boolean = true;

    constructor (public data: TRB_LodInfo_Data) {
        for (let i = 0; i < this.data.mesh_infos.length; ++i) {
            this.mesh_infos.push(new TRB_MeshInfo_Instance(this.data.mesh_infos[i]));
        }
    }

    public calcAnim(deltaTimeInFrames: number): void {
        for (let i = 0; i < this.data.mesh_infos.length; ++i) {
            this.mesh_infos[i].calcAnim(deltaTimeInFrames);
        }
    }

    public calcMtx(): void {
        for (let i = 0; i < this.data.mesh_infos.length; ++i) {
            this.mesh_infos[i].calcMtx();
        }
    }
    public draw(device: GfxDevice, renderInstManager: GfxRenderInstManager, viewerInput: ViewerRenderInput, mesh_resource: TRB_MeshResource_Instance): void {
        const sphere = this.data.lodInfo.header.bounding_sphere;
        const position = vec3.fromValues(sphere[0], sphere[1], sphere[2]);

        for (let i = 0; i < this.data.mesh_infos.length; ++i) {
            this.mesh_infos[i].draw(device, renderInstManager, viewerInput, mesh_resource, position);
        }
    }
}

export class TRB_MeshResource_Instance {
    private lod_infos: TRB_LodInfo_Instance[] = [];

    public transform = mat4.create();
    public joint_matrices: mat4[] = [];

    public visible: boolean = true;
    public name: string;

    constructor (public data: TRB_MeshResource_Data) {
        this.name = data.meshResource.name;
        
        for (let i = 0; i < this.data.lod_infos.length; ++i) {
            this.lod_infos.push(new TRB_LodInfo_Instance(this.data.lod_infos[i]));
        }

        const skeleton = this.data.meshResource.skeleton;

        if (!skeleton)
            return;

        for (let i = 0; i < skeleton.joints.length; ++i) {
            this.joint_matrices.push(mat4.create());
        }
    }
    ...
    public calcAnim(deltaTimeInFrames: number): void {
        for (let i = 0; i < this.data.lod_infos.length; ++i) {
            this.lod_infos[i].calcAnim(deltaTimeInFrames);
        }
    }
    ...
    public calcMtx(viewerInput: ViewerRenderInput): void {
        ...
        if (skeleton) {
            ...
            for (let i = 0; i < joints.length; ++i) {
                const parent_index = joints[i].parent_index;
                if (parent_index >= 0)
                    mat4.mul(this.joint_matrices[i], this.joint_matrices[parent_index], joints[i].unknown1);
                else
                    mat4.mul(this.joint_matrices[i], this.transform, joints[i].unknown1);
            }
        }

        for (let i = 0; i < this.data.lod_infos.length; ++i) {
            this.lod_infos[i].calcMtx();
        }
    }
    ...
    public draw(device: GfxDevice, renderInstManager: GfxRenderInstManager, viewerInput: ViewerRenderInput): void {
        ...
        for (let i = 0; i < this.data.lod_infos.length; ++i) {
            this.lod_infos[i].draw(device, renderInstManager, viewerInput, this);
        }
    }
}
